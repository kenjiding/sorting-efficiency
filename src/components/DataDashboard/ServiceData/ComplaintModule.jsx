...
import { MessageSquare, Search, TrendingUp, Calendar, Filter, BarChart3, Upload, CheckCircle, AlertCircle, X, Sparkles } from 'lucide-react';
...
import { getWeekRange, formatTimePeriod } from '../../../utils/dateUtils';
...
const ComplaintModule = () => {
  ...
  const loadLatestUploadInfo = async () => {
    try {
      const data = await apiClient.serviceData.getComplaintsLatest();
  ...
  const handleAIAnalysis = () => {
    ...
    const prompt = buildServiceDataPrompt(data, dimension, timeUnit, timeRange, 'complaint');
    setAIPrompt(prompt);
    setShowAIModal(true);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between gap-4">
          <h4 className="text-md font-semibold text-gray-900">上传客诉明细文件</h4>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {tableData.map((row, index) => {
                      // 格式化时间显示：周/月维度使用timePeriod，天维度使用createTime
